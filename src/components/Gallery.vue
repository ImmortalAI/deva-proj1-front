<template>
    <div class="w-full">
        <div class="flex gap-4">
            <Button v-if="editorStore.videoFrames.length == 0"
            ...
            <div class="flex flex-wrap max-h-[75vh] overflow-y-scroll">
                <div class="w-48 border-2 border-neutral-500 rounded-xl p-1 m-4 flex items-center"
                    ...
                    <div :key="image.id">
                        ...
                        <div class="relative aspect-video cursor-pointer group" @click="openModal(image)">
        ...
        <!-- Единый диалог вне v-for -->
        <Dialog v-model:visible="isDialogVisible" modal header="Просмотр изображения"
            class="w-full max-w-screen-2xl min-h-[400px]" :dismissableMask="true" :draggable="false" :closable="true"
            :breakpoints="{ '960px': '95vw', '640px': '100vw' }" @hide="onDialogHide">
            <div class="flex flex-col lg:flex-row gap-4 p-6" @click.stop>
                <!-- Левая часть: изображение и кнопки трансформации -->
                <div class="flex flex-col w-full lg:w-2/3 gap-4 justify-between max-h-[80vh]">
                    <div class="flex-grow flex items-center justify-center">
                        <img v-if="activeImage" :src="getImageUrl(activeImage.id)" :alt="activeImage.file_name"
                ...
                <div
                    class="w-full lg:w-1/3 flex flex-col gap-4 bg-neutral-300 dark:bg-neutral-800 rounded-2xl p-6 min-h-[300px]">
                    <!-- Тайм-код -->
                    ...
                    <Button v-if="activeImage" @click="addCommentImage(activeImage.id)" severity="contrast"
                        label="Добавить комментарий" icon="pi pi-comment" />
                    <Button v-if="activeImage" @click="hideImage(activeImage.id)" severity="danger"
                        label="Скрыть изображение" icon="pi pi-eye-slash" />
                </div>
            </div>
        </Dialog>
    </div>
...
const openModal = (image: FileData) => {
    scale.value = 1;          // Сброс масштаба
    rotation.value = 0;       // Сброс поворота
    activeImage.value = image;
    textareaContent.value = image.metadata_text || '';
...
const addCommentImage = async (imgId: string) => {
    ...
    if (index !== -1 && textareaContent.value) {
        ...
        isDialogVisible.value = false; // Закрытие диалога после добавления комментария
    }
};
...
const hideImage = async (imgId: string) => {
    ...
    if (index !== -1) {
        ...
        isDialogVisible.value = false; // Закрытие диалога после скрытия изображения
    }
};

function formatTimecode(image: FileData): string {
